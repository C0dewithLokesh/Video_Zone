...
const Video = () => {
  ...
  return (
    <Stack direction={['column', 'row']} h={'100vh'} marginTop={'70px'}>
        <VStack w={'full'}>
            ...
            <Heading>Sample Video {selectedLectureIndex + 1} </Heading>
        ...
        <VStack w={['full', 'xl']} alignItems={'stretch'} p={'8'} spacing={'8'} overflowY={'auto'} >
            ...
            {videosArr.map((item, index) => (
                    <Button key={index + 1} variant={'ghost'} colorScheme={'purple'} onClick={() => {setVideoSrc(item);  setSelectedLectureIndex(index)}}>
                        Lecture{index + 1}
                    </Button>
                ))}
        </VStack>
